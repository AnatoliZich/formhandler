...
class Validate
{
    ...
    public function main()
    {
        ...
        if ($field) {
            ...
            $formHandlerPath = PathUtility::stripPathSitePrefix(
                \TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extPath('formhandler')
            );

            if ($valid) {
                $content = \Typoheads\Formhandler\Utility\GeneralUtility::getSingle($this->settings['ajax.']['config.'], 'ok');
                if (strlen($content) === 0) {
                    $content = '<img src="' . $formHandlerPath . 'Resources/Public/Images/ok.png' . '" />';
                } else {
                    $gp = [
                ...
                if (strlen($content) === 0) {
                    $content = '<img src="' . $formHandlerPath . 'Resources/Public/Images/notok.png' . '" />';
                } else {
                    $view = $this->initView($content);
                    $gp = [
                        $_GET['field'] => $_GET['value']
